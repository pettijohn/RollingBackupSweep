dotnet publish src -r linux-x64   -c Release -o publish && tar -czf ./publish/RollingBackupSweep-linux-x64.tar.gz ./publish/RollingBackupSweep
...
docker run -it --rm -v .:/code -v /usr/share/dotnet:/runtime -w /code dotnet-linux-arm64-cross \
    ...
    && tar -czf ./publish/RollingBackupSweep-linux-arm64.tar.gz ./publish/RollingBackupSweep